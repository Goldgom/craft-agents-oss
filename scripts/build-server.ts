...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  const rootPkg = {
    name: config.minimal ? 'craft-cli-dist' : 'craft-server-dist',
    version,
    private: true,
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building Craft Agent ${minimal ? 'Minimal CLI' : 'Server'} ${version} for ${platform}-${arch} ===`);
  ...
  if (config.compress) {
    const archiveName = `craft-${minimal ? 'cli' : 'server'}-${version}-${platform}-${arch}.tar.gz`;
