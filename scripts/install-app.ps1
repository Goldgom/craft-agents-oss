# TokenBird Windows Installer
# Usage: irm https://openai.goldgom.top/tokenbird/install-app.ps1 | iex

& {
$ErrorActionPreference = "Stop"

$VERSIONS_URL = "https://openai.goldgom.top/tokenbird/electron"
$DOWNLOAD_DIR = "$env:TEMP\tokenbird-install"
$APP_NAME = "TokenBird"
...
$process = Get-Process -Name "TokenBird" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing TokenBird..."
...
Write-Info "Adding 'tokenbird' command to PATH..."

$binDir = "$env:LOCALAPPDATA\TokenBird\bin"
$cmdFile = "$binDir\tokenbird.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\TokenBird\TokenBird.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'tokenbird' command)"
} else {
    Write-Success "Command 'tokenbird' is ready"
}
...
Write-Host ""
Write-Host "  TokenBird has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: tokenbird (restart terminal first)"
Write-Host ""
}
