...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  // Group once per message snapshot and share the result between search,
  // pagination, and rendering. During streaming this avoids repeating the full
  // O(n) conversation scan for every batched text delta.
  ...
  const matchingOccurrences = useMemo(() => {
    ...
    return matches
  }, [searchQuery, session?.messages, allTurns, countOccurrences])
  ...
  // These settings are stored in ~/.tokenbird/preferences.json (not localStorage)
