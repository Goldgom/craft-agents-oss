...
import { registerStudioScheme, registerStudioHandler } from './studio-protocol'
...
app.whenReady().then(async () => {
  ...
  // Seed tool icons to ~/.tokenbird/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.tokenbird/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
      ...
      const instance = await bootstrapServer<SessionManager, HandlerDeps>({
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            // at ~/.tokenbird/logs/messaging-gateway.log.
