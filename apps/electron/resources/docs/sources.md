...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in TokenBird.
...
- `~/.tokenbird/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.tokenbird/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.tokenbird/workspaces/{ws}/sources/my-source
   ```
