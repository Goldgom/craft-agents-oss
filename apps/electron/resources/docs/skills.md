...
1. **Workspace skill checked first** - If `~/.tokenbird/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.tokenbird/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.tokenbird/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.tokenbird/workspaces/{ws}/skills/commit/SKILL.md`
