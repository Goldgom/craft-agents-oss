...
Terminal client for TokenBird server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running TokenBird headless server with URL and token
