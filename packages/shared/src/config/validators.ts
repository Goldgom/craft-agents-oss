  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.tokenbird/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files (~/.tokenbird/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.tokenbird/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.tokenbird/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.tokenbird/tool-icons/tool-icons.json → tool icon mappings
 */
