#!/usr/bin/env bun
/**
 * Debug script to print the full TokenBird system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                        TOKENBIRD SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. TokenBird Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.tokenbird/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  ...
  '/Users/example/.tokenbird/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.tokenbird/workspaces/abc123/sessions/260121-example-session/plans',
});
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// TokenBird additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. TokenBird Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
