...
// System prompt for TokenBird context
  ...
  switch (code) {
    ...
    case 'CAPABILITY_UNAVAILABLE':
      return 'No connected desktop client supports browser tools, or no client is currently connected. ' +
        'Ask the user to open this workspace from the TokenBird desktop app.';
...
export class PiAgent extends BaseAgent {
  protected backendName = 'TokenBird Backend';
  ...
  private handleSubprocessExit(code: number | null, signal: string | null): void {
    ...
    // Reject pending mini completions / llm_query calls with the diagnostic
    // error (not null) so callers get a meaningful failure instead of silently
    // returning "no response", and clear their deadline timers.
    this.rejectAllPendingEphemeral(new Error(diagnostic));
