...
## Purpose
Core business logic package for TokenBird:
...
- Claude SDK subprocess env is sanitized to strip Claude-specific Bedrock routing vars (`CLAUDE_CODE_USE_BEDROCK`, `AWS_BEARER_TOKEN_BEDROCK`, `ANTHROPIC_BEDROCK_BASE_URL`). Pi Bedrock uses its own AWS env path instead. On Windows the same builder (`agent/options.ts:buildClaudeSubprocessEnv`) also injects `CLAUDE_CODE_GIT_BASH_PATH` from the user-configured `gitBashPath` (`config/storage.ts:getGitBashPath`) when unset, so the SDK's Bash tool honors per-user Git installs instead of only the hardcoded `Program Files` search (#935). The Pi backend's built-in bash tool gets the same path via the init message (`agent/pi-agent.ts` sends `gitBashPath`; `pi-agent-server` resolves it in `bash-shell-path.ts` and passes it as `BashToolOptions.shellPath` to `createBashToolDefinition` — the Pi SDK has no env-var override of its own).
- Pi subprocess teardown must kill the whole process tree, never just the direct child. On Windows `child.kill('SIGTERM')` is `TerminateProcess`: it skips the subprocess's shutdown cleanup AND leaves the bash tool's grandchildren (bash.exe → python.exe/bun.exe/...) orphaned as zombies. Use `killProcessTree`/`killProcessTreeAsync` (`utils/process-tree.ts`, taskkill `/T /F` on Windows, process-group SIGKILL on POSIX) from `agent/pi-agent.ts` (`killSubprocess`, `killSubprocessGracefully`). The subprocess itself defers `process.exit` by a short grace period after `piSession.dispose()` (`pi-agent-server/src/index.ts:exitAfterCleanup`) so the SDK's async child kills complete first.
...
3. **Use the localized product name**: TokenBird in non-Chinese locales, 词元鸟 in Simplified Chinese, and 詞元鳥 in Traditional Chinese. Keep technical names such as Claude, Anthropic, OpenAI, MCP, API, and SDK in English.
...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.tokenbird/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
