 ...
 *   --workspace-root: Path to workspace folder (~/.tokenbird/workspaces/{id})
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.tokenbird/workspaces/{id}
      // preferencesPath = ~/.tokenbird/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.TOKENBIRD_CONFIG_DIR || join(workspaceRootPath, '..', '..');
