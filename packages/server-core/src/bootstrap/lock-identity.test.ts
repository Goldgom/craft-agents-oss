...
describe('parseTasklistImageName', () => {
  it('extracts the image name from a CSV data row', () => {
    expect(parseTasklistImageName('"TokenBird.exe","1234","Console","1","150,000 K"')).toBe('TokenBird.exe')
  })
...
describe('lockHolderMatchesLock (#978)', () => {
  ...
  it('rejects a different executable even when its name contains "craft"', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('TokenBird.exe'), 'minecraft-launcher', null)).toBe(false)
  })

  it('fails open when the live exec name cannot be determined', () => {
    expect(lockHolderMatchesLock(lockWith('TokenBird.exe'), null, null)).toBe(false)
  })
  ...
  it('ignores the command line when execName is recorded', () => {
    ...
    expect(lockHolderMatchesLock(lockWith('TokenBird.exe'), 'java', '/usr/bin/java -jar minecraft.jar')).toBe(false)
  })
