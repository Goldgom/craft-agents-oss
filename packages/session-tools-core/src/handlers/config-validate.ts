 ...
 *
 * Validates TokenBird configuration files.
...
export async function handleConfigValidate(
  ...
  const tokenBirdRoot = process.env.TOKENBIRD_CONFIG_DIR || join(homedir(), '.tokenbird');
  ...
  switch (target) {
    case 'config': {
      const result = validateJsonFileHasFields(
        join(tokenBirdRoot, 'config.json'),
        ['workspaces']
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'preferences': {
      const result = validateJsonFileHasFields(
        join(tokenBirdRoot, 'preferences.json'),
        []
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'tool-icons': {
      const result = validateJsonFileHasFields(
        join(tokenBirdRoot, 'tool-icons', 'tool-icons.json'),
        ['version', 'tools']
      );
      return successResponse(formatValidationResult(result));
    }
